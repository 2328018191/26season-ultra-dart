...
void OSTaskInit()
{
    ...
    osThreadDef(VisionSend, StartVISIONTASK, osPriorityNormal, 0, 1024);
    uiTaskHandle = osThreadCreate(osThread(VisionSend), NULL);

    HTMotorControlInit(); // 没有注册HT电机则不会执行
}

// vision_send
__attribute__((noreturn)) void StartVISIONTASK(void const *argument)
{
    static float send_start;
    static float send_dt;
    LOGINFO("[freeRTOS] INS Task Start");
    for (;;)
    {

        send_start = DWT_GetTimeline_ms();
        send_dt = DWT_GetTimeline_ms() - send_start;
        if (send_dt > 1)
            LOGERROR("[freeRTOS] INS Task is being DELAY! dt = [%f]", &send_dt);
        // McuToNuc();
