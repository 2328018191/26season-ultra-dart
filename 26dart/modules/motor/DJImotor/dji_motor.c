...
DJIMotorInstance *DJIMotorInit(Motor_Init_Config_s *config)
{
    ...
    instance->motor_controller.other_force_feedback_ptr =config->controller_param_init_config.other_force_feedback_ptr;
...
void DJIMotorControl()
{
    ...
    { // 减小访存开销,先保存指针引用
        ...
        if (motor_setting->force_feedback_source == OTHER_FEED)
            pid_measure = *motor_controller->other_force_feedback_ptr;
            
        if (motor_setting->close_loop_type & CURRENT_LOOP)
        {
             motor->debug.current_loop_measure = pid_measure;
            pid_ref = PIDCalculate(&motor_controller->current_PID, pid_measure, pid_ref);
        }

        if (motor_setting->feedback_reverse_flag == FEEDBACK_DIRECTION_REVERSE)
            pid_ref *= -1;
        motor->debug.current_set    = (int16_t)pid_ref;  // 真正输出
        motor->debug.current_measure= pid_measure;       // 实际采样
        motor->debug.pid_ref        = motor_controller->pid_ref; // 原始给定
        // 获取最终输出
        set = (int16_t)pid_ref;
