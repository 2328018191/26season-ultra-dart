...
{
     struct {
        float angle_loop_measure;
        float speed_loop_measure;
        float current_loop_measure;
        int16_t  current_set;     // ← 真正发出去的电流值
        float    current_measure; // ← 电流环采样值（力反馈）
        float    pid_ref;         // ← 电流环给定
    } debug;
    ...
    float dt;
    //滑摩控制器
    SMC_Config smc_accel_config;
    SMC_Config smc_steady_config;
